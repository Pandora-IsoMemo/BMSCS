...
importData <- function(input, output, session,
  ...
  observeEvent({
    ...
    ## Import valid?
    val <- if (is.reactive(validateData)) validateData()
    else validateData

    res <- try(val(df), silent = TRUE)

    if (inherits(res, "try-error") || !is.list(res)){
      values$fileImportWarning <- c(
        values$fileImportWarning,
  ...
  output$warning <- renderText(paste(values$fileImportWarning, sep = "\n"))
