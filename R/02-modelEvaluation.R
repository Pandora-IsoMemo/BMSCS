modelEvaluationTab <- function(id) {
    ...
    tabPanel(
        ...
        radioButtons(ns("ic"), "Information / Cross-Validation Error criterion",
            choices = c("Loo", "WAIC", "AIC", "AICc", "BIC", "logLik", "Rsq", "RsqAdj", "df", "MallowsCP"), width = "100%"
        ),
...
modelEvaluation <- function(input, output, session, model) {
    observe({
        ...
        if (model()$models[[1]]@type == "linear") {
            updateRadioButtons(session, "ic",
                choices = c("Loo", "WAIC", "AIC", "AICc", "BIC", "logLik", "Rsq", "RsqAdj", "df", "MallowsCP")
            )
        }
        if (model()$models[[1]]@type == "logistic") {
            updateRadioButtons(session,
                "ic",
                choices = c("Loo", "WAIC", "AIC", "AICc", "BIC", "logLik", "Rsq", "AUC", "df", "nagelkerke")
            )
        }
    })
    ...
    dataFun <- reactive({
        ...
        function() {
            ...
            names(fits) <- input$ic
            if(input$ic %in% c("AUC", "Rsq", "RsqAdj", "df", "logLik", "nagelkerke", "Loo", "WAIC")){
